...
export class ClSimplexSolver extends ClTableau {
    ...
    addAssignmentConstraints() {
        ...
        //console.log('foobaz', this._constraintVariablesByVariables, this._constraintVariablesByVariables.length)
        this._constraintVariablesByVariables.forEach(constraintVariable => {
            console.log(`check add assignment to variable ${constraintVariable.toString()}`, constraintVariable._lastValue);
            if(constraintVariable._lastValue === constraintVariable._value) {
                constraintVariable.stayConstraint || constraintVariable.stay(ClStrength.weak);
                console.log(`add stay constraint: ${constraintVariable.toString()}`);
            } else {
                //this.suggestValue(constraintVariable, constraintVariable.value());
                constraintVariable.stayConstraint && constraintVariable.removeStay();
                let c = constraintVariable.cnEquals(constraintVariable.value());
                console.log(`added equality constraint for ${constraintVariable}: ${c}`);
                this._temporaryAssignmentConstraints.push(c);
                this.addConstraint(c);
            }
            // return;
            // console.log(constraintVariable.toString(), constraintVariable._lastValue);
            // if(constraintVariable._lastValue !== constraintVariable._value) {
            //     console.log(`${constraintVariable._name} is read only}`);
            //     constraintVariable.setReadonly(true);
            // }
            // console.log('--', constraintVariable.toString(), constraintVariable._lastValue);
        });
        //console.log(this.toString());
    ...
    removeAssignmentConstraints() {
        console.log('remove assignments');
        this._constraintVariablesByVariables.forEach(constraintVariable => {
            console.log(`${constraintVariable.toString()}`);
        });
        ...
        this._constraintVariablesByVariables.forEach(constraintVariable => {
            console.log(`${constraintVariable.toString()} has stay?: ${!!constraintVariable.stayConstraint}`);
            constraintVariable.removeStay();
